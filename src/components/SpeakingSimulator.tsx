...
export const SpeakingSimulator: React.FC<SpeakingSimulatorProps> = ({
  ...
  const playBeep = () => {
    try {
      ...
      gainNode.gain.value = 0.05; // Громкость 5%
      
      osc.type = 'sine';
      osc.frequency.setValueAtTime(800, ctx.currentTime);
      
      osc.connect(gainNode); // Подключаем звук к регулятору
      gainNode.connect(ctx.destination); // Выводим на динамики
          ...
          <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/40 text-center space-y-3 animate-in fade-in">
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 font-bold text-xs rounded-xl transition-colors border border-rose-500/30 flex items-center space-x-1.5"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-slate-700 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-colors flex items-center space-x-1.5"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-sky-500 hover:bg-sky-600 text-white font-bold text-xs rounded-xl shadow-md transition-colors flex items-center space-x-1.5"
              >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className={`w-full max-w-sm p-6 rounded-3xl shadow-2xl border text-center ${isDarkMode ? 'bg-[#1e2c3a] border-slate-700' : 'bg-white border-slate-200'}`}>
            ...
            <h3 className={`text-base font-bold mb-2 ${isDarkMode ? 'text-slate-200' : 'text-slate-800'}`}>Удалить эту запись?</h3>
            <p className="text-xs text-slate-400 mb-6">Это действие нельзя отменить. Ваша запись будет удалена навсегда.</p>
            <div className="flex items-center space-x-3">
              <button
                onClick={() => setShowDeleteConfirm(false)}
                className="flex-1 py-3 rounded-xl font-bold text-xs bg-slate-800 text-slate-300 hover:bg-slate-700 transition-colors"
              >
                Отмена
              </button>
              <button
                ...
                className="flex-1 py-3 rounded-xl font-bold text-xs bg-rose-600 text-white hover:bg-rose-700 transition-colors shadow-lg"
              >
                Да, удалить
              </button>
            </div>
